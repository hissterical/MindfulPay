...
const ProfileScreen: React.FC = () => {
  ...
  // State for modals
  const [blockedMerchantsModalVisible, setBlockedMerchantsModalVisible] =
    useState(false);
  const [newMerchant, setNewMerchant] = useState("");

  // Handle adding a new blocked merchant
  const handleAddBlockedMerchant = async () => {
    if (!newMerchant.trim()) {
    ...
    try {
      // Add to both FinancialContext and vendor check system
      await addBlockedMerchant(newMerchant.trim());
      await addToBlocklist(newMerchant.trim());
      setNewMerchant("");
      Alert.alert("Success", "Merchant added to blocklist");
    } catch (error) {
      console.error("Error adding blocked merchant:", error);
      Alert.alert("Error", "Failed to add merchant to blocklist");
    }
  };

  // Handle removing a blocked merchant
  const handleRemoveBlockedMerchant = async (merchant: string) => {
    try {
      // Remove from both FinancialContext and vendor check system
      await removeBlockedMerchant(merchant);
      await removeFromBlocklist(merchant);
      Alert.alert("Success", "Merchant removed from blocklist");
    } catch (error) {
      console.error("Error removing blocked merchant:", error);
      Alert.alert("Error", "Failed to remove merchant from blocklist");
    }
  };
      ...
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          <View style={styles.modalHeader}>
            ...
            <TouchableOpacity
              onPress={() => setBlockedMerchantsModalVisible(false)}
            >
            ...
            {blockedMerchants.length > 0 ? (
              blockedMerchants.map((merchant, index) => (
                <View key={index} style={styles.merchantItem}>
                  ...
                  <TouchableOpacity
                    onPress={() => handleRemoveBlockedMerchant(merchant)}
                  >
          ...
          >
            <View style={styles.settingsItemLeft}>
              <Ionicons
                ...
                style={styles.settingsIcon}
              />
              <Text style={[styles.settingsItemText, { color: "#F44336" }]}>
                Clear All Data
              </Text>
            </View>
            <Ionicons name="chevron-forward" size={20} color="#999" />
